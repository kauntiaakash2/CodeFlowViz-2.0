...
export default function SharedSessionPage() {
  ...
  useEffect(() => {
    ...
    // Reset stale data immediately so a previous session never shows during load.
    setSessionData(null);
    setError(null);
    ...
    const fetchSession = async () => {
      try {
        ...
        const response = await fetch(sessionsApiUrl, { signal: controller.signal });
        if (!response.ok) {
        ...
        // Only clear loading state if this effect is still the active one.
    ...
    // Abort the in-flight request when sessionId changes or component unmounts.
  ...
  return (
    <PlaybackProvider initialSession={sessionData}>
      <main className="shell">
        <header className="topbar">
          <div>
            <p className="eyebrow">CodeFlowViz 2.0</p>
