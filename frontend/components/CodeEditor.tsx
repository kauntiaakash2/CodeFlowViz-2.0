...
export default function CodeEditor() {
  const { code, setCode, output, isRunning, runCode, snapshots, playback } =
    usePlayback();

  const { selectedSnapshotIndex, setSelectedSnapshotIndex } = playback;
  ...
  const selectedSnapshot =
    selectedSnapshotIndex === null
      ? null
      : (snapshots[selectedSnapshotIndex] ?? null);
  const selectedVariables = selectedSnapshot
  ...
  const onBottomSashKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLDivElement>) => {
      if (!containerRef.current || isCollapsed || isMaximized) {
        return;
      }

      const step = event.shiftKey ? 20 : 10;

      let nextHeight = bottomHeight;
    ...
    [bottomHeight, isCollapsed, isMaximized],
  );
  const onRightSashMouseDown = useCallback(
    (e: React.MouseEvent) => {
      e.preventDefault();
      e.stopPropagation();
      setIsSashDragging(true);
  ...
  const onRightSashKeyDown = useCallback(
  (event: React.KeyboardEvent<HTMLDivElement>) => {
    if (!containerRef.current || isCollapsed || isMaximized) {
      return;
    }
  ...
  [rightWidth, isCollapsed, isMaximized]
);
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
          ...
          <button
            ...
            aria-label={
              dockPosition === "bottom"
                ? "Move output panel to the right"
                : "Move output panel to the bottom"
            }
          ...
          {isMaximized || isCollapsed ? (
            <button
              ...
              onClick={resetPanel}
              title="Restore"
              aria-label="Restore output panel"
              style={quickBtnStyle}
            >
              🗗
            </button>
        ...
        <div className="outputBody">
          {output ? (
            <>
              ...
              {output.result ? (
                <pre>
                  Result ({output.result.type}): {output.result.value}
                </pre>
              ) : null}
              {snapshots.length ? (
                <>
                  <section
                    className="scrubberPanel"
                    aria-label="Execution playback scrubber"
                  >
                    <div className="scrubberMeta">
                      ...
                      {selectedSnapshot ? (
                        <span>
                          step #{selectedSnapshot.step} · line{" "}
                          {selectedSnapshot.line} · {selectedSnapshot.event}
                        </span>
                      ) : null}
                    </div>
                    <input
                  ...
                  >
                    <div className="inspectorHeader">
                      ...
                      {selectedSnapshot ? (
                        <span>Active line {selectedSnapshot.line}</span>
                      ) : null}
                    </div>
                  ...
                  <ol
                    ...
                    aria-label="Execution trace snapshots"
                  >
                      ...
                      <li key={snapshot.step}>
                        ...
                        >
                          <span className="stepMeta">
                            #{snapshot.step} · line {snapshot.line} ·{" "}
                            {snapshot.event}
                          </span>
                        </button>
                      </li>
            ...
            <p style={{ padding: "0.5rem", margin: 0 }}>
              Run code to see variable snapshots, loop checkpoints, console
              output, errors, and timeout status.
            </p>
          )}
        </div>
      )}
    ...
    return (
      ...
      >
        ...
        <div
          ...
          role="separator"
          tabIndex={isMaximized || isCollapsed ? -1 : 0}
  ...
  aria-controls="output-panel-right"
  aria-valuemin={200}
  aria-valuemax={
    containerRef.current
  ...
  onKeyDown={onRightSashKeyDown}
  style={sashStyle(true)}
/>

      {/* Right — output */}
      <div
      id="output-panel-right"
        className={`outputPane ${output?.ok ? "success" : output ? "failure" : ""}`}
        style={{
          width: `${rightWidth}px`,
          minWidth: 0,
